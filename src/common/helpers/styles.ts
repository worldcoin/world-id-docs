export const styles = {
  screenPadding:
    'px-4 md:px-8 lg:px-8 xl:px-16 2xl:px-[calc((100vw-1540px)/2)]',
  gradient: 'bg-gradient-to-r from-fff0ed to-edecfc',
  textGradient:
    'text-transparent bg-clip-text bg-gradient-to-r from-fff0ed to-edecfc',
  darkTextGradient: 'text-transparent bg-clip-text from-fff0ed to-edecfc',
  headerButton:
    'p-1 border rounded-lg border-262f41/10 bg-1a2436/5 text-94a2b8',
  fenceBorder:
    'absolute -inset-px -z-10 rounded-[16px] bg-[image:linear-gradient(35deg,_rgba(234,_209,_202,_0.6),_rgba(222,_209,_204,_0.05)_25%,_rgba(203,_209,_207,_0.6)_66%,_rgba(181,_208,_231,_0.14))]',
  fenceBorderInner: 'relative [clip-path:inset(1px_round_16px)] rounded-[16px]',
  background: 'bg-ebedef',
}
