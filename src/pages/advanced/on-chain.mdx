...
function verifyAndExecute(
    ...
    worldId.verifyProof(
        root,
        1, // Or 0 if the user is Phone Verified
...
The World ID protocol currently lives in the Polygon network, and the verification router contract is deployed at `polygon.worldcoin.eth` on Polygon mainnet and `mumbai.id.worldcoin.eth` on Polygon Mumbai testnet.
