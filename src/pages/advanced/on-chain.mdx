...
We provide starter kits for both [Foundry](https://github.com/worldcoin/world-id-starter) and [Hardhat](https://github.com/worldcoin/world-id-starter-hardhat) to help you get started with World ID. We strongly recommend using one of these starter kits to get started with World ID.
...
For example, a smart contract performing an airdrop might include the receiver address in the signal, while a contract allowing users to vote on a governance proposal might include the proposal ID and the vote.
...
- After a user has successfully verified a proof, add their `nullifierHash` to the list of used `nullifierHash`es.
...
/// @param proof The zero-knowledge proof that demostrates the claimer is registered with World ID (returned by the IDKit widget).
function verifyAndExecute(
    ...
    worldId.verifyProof(
        ...
        nullifierHash,
        abo.encodePacked(appId).hashToField(),
        proof
    );
