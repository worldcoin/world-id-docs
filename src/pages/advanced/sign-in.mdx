...
After registration is complete, you will have a valid `app_id` that will be needed for every other step in the authentication process. This value is equivelent to `client_id` from the [OIDC specification](https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest)
...
All attempts at authentication begin with a request to the `/authorize` endpoint. Request details can be [found here](#/authorize).

### Hosted SIWI

When using the native Sign In with World ID page, much of the OIDC process is handled for you. You can begin the authentication cycle by redirecting your users to:

`https://id.worldcoin.org/login?client_id={app_id}&response_type={code|token|id_token}&redirect_uri={encoded_redirect_url}&state={state_value}&nonce={nonce_value}`

Example values could be:

- `client_id`: "app_123456", obtained from registration step
- `response_type`: "code%20id_token", valid response types from OIDC spec
- `redirect_uri`: "https%3A%2F%2Fapp.example.com%2Flogin", must be on the approved redirect URI list from registration
- `state`: "session_123", unique value used to track a user's session
- `nonce`: "1234567890", random value to verify id_token's authenticity

The user will then authenticate with their World ID via the World app. The app generates a Zero-Knowledge proof proving that the user has verified their phone number or in-person with an Orb. This proof can be verified by calling the [`/verify` endpoint](TODO). Once successfully authorized, the user is redirected back to your application.

The redirect URL will contain a number of values, depending on the flow you are using.
