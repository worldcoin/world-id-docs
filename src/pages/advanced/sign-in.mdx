...
2. Use the `/register` endpoint, request body details can be [found here](TODO)
...
All attempts at authentication begin with a request to the `/authorize` endpoint. Request details can be [found here](TODO).

When using the native Sign In with World ID page, much of the process is handled for you. You can begin the authentication cycle by redirecting your users to:

`https://id.worldcoin.org/login?client_id={app_id}&response_type={code|token|id_token}&redirect_uri={encoded redirect URL}&state={state value}&nonce={nonce value}`
...
- `state`: "session_123", unique value used to track user session
- `nonce`: "1678318593", random value to verify id_token's authenticity
...
- `code`: An authorization code that can be

If you received a response containing an _authorization code_, you must exchange it for an ID token on the `/token` endpoint. Request details can be [seen here](TODO).
