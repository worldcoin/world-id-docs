...
### Required attributes
    ...
    <Properties>
      ...
      <Property name="merkle_root" type="string">
        The ZKP's Merkle root, as provided by IDKit. See [IDKit response](/idkit/reference#response) for details.
      </Property>
...
```bash {{ title: "cURL" }}
curl -X POST "https://id.worldcoin.org/api/v1/verify/{app_id}" \
...
```bash {{ title: "cURL" }}
curl -X POST "https://id.worldcoin.org/api/v1/precheck/{app_id}" \
     -H "Content-Type: application/json" \
     -d '{
...
```js
fetch(`https://id.worldcoin.org/api/v1/precheck/${appId}`, {
	method: 'POST',
	headers: {
		'Content-Type': 'application/json',
	},
...
```bash {{ title: "cURL" }}
curl https://developer.worldcoin.org/api/v1/jwks
```

```js
fetch('https://developer.worldcoin.org/api/v1/jwks')
```
...
```bash {{ title: "cURL" }}
curl -X POST "https://id.worldcoin.org/v1/graphql" \
     -H "Content-Type: application/json" \
...
```js
fetch('https://id.worldcoin.org/v1/graphql', {
	method: 'POST',
	headers: {
		'Content-Type': 'application/json',
