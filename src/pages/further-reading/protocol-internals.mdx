...
1. IDKit initiates the proof request session on the World ID Bridge.
    ...
    - IDKit generates an AES-GCM `key` and `iv`, used to encrypt the verification request. IDKit temporarily stores the `key` and `iv` in memory.
    ...
    - IDKit sends a POST request to the Bridge: `https://bridge.worldcoin.org/request`, with the JSON body described below, thus initiating the session and sending the payload required to generate a proof. The Bridge returns a UUIDv4 `requestId` to IDKit, which is used to identify the request.
        ```json
        {
            "app_id": "app_123456", // starts with "app_", or is equal to "self_hosted"
            "action": "some_string",
            "signal": "0xa1ddac5b45704c897203...", // optional
            "credential_types": ["orb", "device"], // optional
            "action_description": "Blah blah blah", // optional
            "return_to": "https://example.com" // only used when deeplinking to mobile application
        }
        ```
...
3. IDKit polls the Bridge for the response from the client, receives the encrypted response, decrypts it, and calls the callback with the proof or handles the returned error.
    - IDKit sends a GET request to `https://bridge.id.worldcoin.org/response/${requestId}` to check if the proof is available. The JSON returned will include a `status` field with one of the following values:
