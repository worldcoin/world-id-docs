...
Your backend should receive the `proof`, `merkle_root`, `nullifier_hash`, and `verification_level` returned by IDKit, as well as the `signal` that was input into IDKit, and send it to the **Developer Portal API** for verification.
...
After performing your own backend actions based on this result, you then pass the success or error messages back to your frontend.
...
<Note>
	For more information on configuration, see the [IDKit](/reference/idkit) and [Cloud API](/reference/api) reference
	pages.
</Note>
