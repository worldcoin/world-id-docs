...
Our backend should receive the `proof`, `merkle_root`, `nullifier_hash`, and `verification_level` returned by IDKit, as well as the `signal` that was input into IDKit, and send it to the **Developer Portal API** for verification.
