...
<Tabs>
    ...
    <TabItem label="Custom Authentication Engines">
      ...
      - **Response type**: depends on how you want to authenticate, follow the recommendations of your engine. We recommend authorization code flow for server-side apps and implicit flow for client-side apps.
        ...
        - `id_token` (recommended) or `id_token token` for implicit flow (ensure your authentication engine provides and validates a `nonce` when using the implicit flow)
        - `code id_token`, `code token`, or `code token id_token` for hybrid flow. Generally not recommended for Sign in with Worldcoin.

      <Note>
        We use the same JWT token as the `id_token` and `access_token`. There are no actions or information accessible with the `access_token` that are not contained within the `id_token`, as Sign in with Worldcoin is intended only for authentication and not authorization.
