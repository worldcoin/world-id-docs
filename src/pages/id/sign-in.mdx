...
-   `response_type`: **Optional**, the response type expected by the application. Can include any of "code", "token", or "id_token". Defaults to "code"
    -   `code`: The [`/authorize`](#generate-credentials) endpoint will include an authorization code that can be exchanged for an access token and/or ID token on the [`/token`](#exchange-code) endpoint
    -   `token`: The [`/authorize`](#generate-credentials) endpoint will include an access token, which can be used to fetch user information from the [`/userinfo`](#get-user-info) and [`/introspect`](#introspect) endpoints
    -   `id_token`: The [`/authorize`](#generate-credentials) endpoint will include a signed JWT containing information about the user, including the unique `sub` claim (otherwise known as the [nullifier hash](/idkit/reference#response))
