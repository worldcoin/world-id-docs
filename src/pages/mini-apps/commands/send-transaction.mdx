...
Users can sign and send multiple transactions in parallel, without needing to wait for previous transactions to be signed or mined. Sequential execution is not enforced for miniapp transactions, unless those transactions are placed in the transaction array and sent as a single sendTransaction command.
