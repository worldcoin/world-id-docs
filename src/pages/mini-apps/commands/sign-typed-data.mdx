...
> 💡 **Note:** The `domain` must match the Safe’s EIP-712 domain. User wallets are [Safe](https://safe.global/) smart contract wallets.
>
> ✅ `EIP712Domain` must always include only:
>
> -   `chainId`
> -   `verifyingContract` (the user's wallet address, typically the Safe address)
>
...
```ts
{
  ...
  domain: {
    chainId: 480,
    verifyingContract: userWallet, // The Safe address
  },
