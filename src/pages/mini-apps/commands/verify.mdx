...
## Sending the command
Minikit uses a slightly different input payload than IDKit.
``` tsx
...
The main change to incognito actions is the removal of `app_id` from the payload. Since World App already knows your `app_id` when it loads the mini app, there's no need to pass it. 
