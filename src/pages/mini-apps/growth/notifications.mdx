...
2. **Personalize** – `${username}` or contextual data lifts opens dramatically.
3. **Deep‑link** – Always pass `mini_app_path` so a tap lands on the exact screen.
4. **Respect timing** – Local daytime hours only; no spam.
5. **Copy rules** – ≤ 30‑char title, 1–2 emojis, clear value + curiosity gap.
...
Track opens (primary), time‑to‑open, and downstream action in the Developer Portal.
