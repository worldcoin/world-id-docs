...
1. Create an action for your app in the Developer Portal. Each action has a unique identifier which you set (e.g. `vote_1`).
...
2. Install the [JS widget](/js/) in your frontend.
...
3. Import and render IDKit. You'll want to do this on the screen where the user executes the protected action (e.g. before they click "Claim airdrop" or "Vote on proposal").
...
4. Verify your action by sending the zero knowledge proof to the **Developer Portal API**. Please check the [API reference](/api/reference#verify) for details on anticipated error responses.
