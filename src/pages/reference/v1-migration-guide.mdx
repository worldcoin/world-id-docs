...
<Note>You can view the [Wallet Bridge](https://github.com/worldcoin/wallet-bridge) GitHub repository or [Protocol Internals](/further-reading/protocol-internals#wallet-bridge) page to learn more.</Note> 

There is a new `bridge_url` parameter in IDKit defining the URL of the Wallet Bridge to use, which must be a valid HTTPS URL. The default value is `https://bridge.worldcoin.org`. 
...
<IDKitWidget
	...
	action="vote_1"
	onSuccess={onSuccess}
    bridge_url="https://bridge.worldcoin.org" // Default value
>
	{({ open }) => <button onClick={open}>Verify with World ID</button>}
</IDKitWidget>
```
...
<Note>Read more about the `onError` callback in the [IDKitWidget reference documentation](/reference/idkit#parameters).</Note>
